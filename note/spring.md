...
* prototype 多例
  每一次请求（或将其注入到另一个bean中、以程序的方式调用容器的 getBean()方法）都会产生一个新的bean实例，相当与一个new的操作。
  Spring不对一个prototype bean的整个生命周期负责，容器在初始化、配置、装饰或者是装配完一个prototype实例后，将它交给客户端，随后就对该prototype实例不闻不问了。
  不管何种作用域，容器都会调用所有对象的初始化生命周期回调方法，而对prototype而言，任何配置好的析构生命周期回调方法都将不会被调用。 
  清除prototype作用域的对象并释放任何prototype bean所持有的资源，都是客户端代码的职责。
  让Spring容器释放被singleton作用域bean占用资源的一种可行方式是，通过使用 bean的后置处理器，该处理器持有要被清除的bean的引用。
* request
  表示该针对每一次HTTP请求都会产生一个新的bean，同时该bean仅在当前HTTP request内有效。
* session
  表示该针对每一次HTTP请求都会产生一个新的bean，同时该bean仅在当前HTTP session内有效。
* global session
  类似于标准的HTTP Session作用域，不过它仅仅在基于portlet的web应用中才有意义。Portlet规范定义了全局Session的概念，它被所有构成某个 portlet web应用的各种不同的portlet所共享。
  在global session作用域中定义的bean被限定于全局portlet Session的生命周期范围内。如果你在web中使用global session作用域来标识bean，那么web会自动当成session类型来使用。


#### 注解
* @Controller 修饰类，将该类标识为Handler；

* @RestController 修饰类，该类下的请求都是REST风格的请求；
...
* @Aspect 表示该类为AOP切面处理类

* @Component  表示该类交由springIOC管理其生命周期


************************************************************************************************************************
### SpringBoot
#### 多环境场景部署服务
* 命令行启动，通过 java -jar xxx.jar 方式来启动 springboot 服务，需要项目以jar包的形式编译好。
  该方式可以指定应用中的参数，例如 java -jar xxx.jar --server.port=8888 指定服务端口为8888。
  连续的两个减号－－就是对 application.properties 中的属性值重新进行赋值，优先级高于代码中的配置(具体的优先级见下文)。

* SpringBoot对数据文件的加载优先级，从高到底如下：
    * 1.在命令行中传入的参数。
...
* SpringBoot 针对多环境多配置的常见解决方案
    * 配置 application-{profile}.properties 文件，针对不同的环境建立不同的配置文件，再通过设置 spring.profiles.active 属性值来使用不同的环境配置文件。
        * 例如 spring.profiles.active=test 时会加载，application-test.properties 这配置文件的值，而其他 application-{profile}.properties 文件的值不会被加载。
        * 具体的实操方案有：
          ①.项目是用 maven 来进行包管理，编译打包时指定环境变量，spring.profiles.active 值配置为 @env@
          ②.命令行启动时使用 java -jar xxx.jar --spring.profiles.active=test 来指定使用 test 环境的配置文件
    * 利用上文中加载优先级别，可以利用 7、8 或者 9、10 这几个特性，来做到在响应的环境服务器中通过外置配置文件，来实现不同环境不同配置，且该方案中具体的配置内容仅对各个环境的运维人员可见，安全性更好。


#### application.yml示例
```yaml
### https://blog.csdn.net/lpfsuperman/article/details/78287265###;
### 具体是spring boot那个版本还不清楚
