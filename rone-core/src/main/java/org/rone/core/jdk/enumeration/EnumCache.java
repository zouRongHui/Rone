...
/**
 * 枚举类的缓存
 * 用来解决根据枚举的某个属性获取响应的枚举类的场景，解决了每个枚举类都要自己编写效率低下的 getByCode() 方法，{@link RoneEnum#getByCode(Integer)}
 * @author zouRongHui
 * @date 2023/6/12
 */
public class EnumCache {
     ...
     * 以枚举任意值构建的缓存结构
     **/
    public static final Map<Class<? extends Enum>, Map<Object, Enum>> CACHE_BY_VALUE = new ConcurrentHashMap<>();
    /**
     * 以枚举名称构建的缓存结构
     **/
    public static final Map<Class<? extends Enum>, Map<Object, Enum>> CACHE_BY_NAME = new ConcurrentHashMap<>();
    /**
     * 枚举静态块加载标识缓存结构
     */
    public static final Map<Class<? extends Enum>, Boolean> LOADED = new ConcurrentHashMap<>();
    ...
    public static <E extends Enum> void registerByName(Class<E> clazz, E[] es) {
        ...
        for (E e : es) {
            // 这里无需像 registerByValue() 中检查是否重复，因为 name 唯一
            map.put(e.name(), e);
        }
